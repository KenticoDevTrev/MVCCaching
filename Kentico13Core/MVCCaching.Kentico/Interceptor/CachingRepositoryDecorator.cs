...
namespace MVCCaching.Kentico
{
    ...
    public sealed class CachingRepositoryDecorator : IInterceptor
    {
        ...
        {
            if (!cachingRepoContext.CacheEnabled() || !invocation.Method.IsPublic)
            {
                invocation.Proceed();
        ...
        {
            return new CacheSettings(cachingRepoContext.DefaultCacheItemDuration().TotalMinutes, cacheKey)
            {
